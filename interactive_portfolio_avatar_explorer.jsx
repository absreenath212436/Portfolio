...
// PortfolioWithAvatar.jsx
// Single-file React component (Tailwind + Framer Motion). Default export is the page.
// features:
// - Stylized SVG avatar (customizable) that "resembles" the user via simple controls: skin tone, hair, facial hair, glasses
// - Avatar follows clicks: click a section and avatar animates to that section
// - Small idle animations (blink, bob) and cursor-following head tilt
// - Upload photo input is provided so you can optionally upload a photo to replace the avatar image (note: you can implement back-end face-mapping later)
...
export default function PortfolioWithAvatar() {
  ...
  // map section to x position (percent)
  ...
  useEffect(() => {
    // animate avatarX when avatarSection changes
    const target = sectionPositions[avatarSection] ?? 8;
    setAvatarX(target);
  }, [avatarSection]);

  // cursor-based head tilt
  useEffect(() => {
    const handleMove = (e) => {
      if (!containerRef.current) return;
      const rect = containerRef.current.getBoundingClientRect();
      const x = (e.clientX - rect.left) / rect.width; // 0..1
      const tilt = (x - 0.5) * 20; // -10..10 deg
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="flex gap-3 items-center">
            {['home','about','experience','projects','contact'].map(s => (
              <button key={s} onClick={() => handleSectionClick(s)} className="text-sm px-3 py-2 hover:bg-slate-100 rounded-md">{s.charAt(0).toUpperCase()+s.slice(1)}</button>
            ))}
      ...
      <main ref={containerRef} className="max-w-6xl mx-auto px-6 py-10">
        ...
        <section className="relative bg-white rounded-2xl shadow-md p-6 overflow-hidden">
          ...
          <div className="relative h-40 flex items-end">
            ...
            <div className="absolute inset-x-4 bottom-0 h-0.5 bg-slate-100">
              {Object.keys(sectionPositions).map((sec) => (
                <div key={sec} style={{ left: `${sectionPositions[sec]}%` }} className="absolute -top-1 w-2 h-2 rounded-full bg-slate-300 transform -translate-x-1/2" />
              ))}
            </div>
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="col-span-2">
              ...
              <p className="mt-3 text-slate-600">A Senior Process Manager with 8 years of experience in product operations and process excellence. Click sections above to explore — the avatar will move with you.</p>
            ...
            <div className="p-4 rounded-lg bg-white border border-slate-100">
              ...
              <input type="color" value={skinTone} onChange={(e) => setSkinTone(e.target.value)} className="w-full h-10 rounded-md my-2" />
              <label className="text-xs">Hair color</label>
              <input type="color" value={hairColor} onChange={(e) => setHairColor(e.target.value)} className="w-full h-10 rounded-md my-2" />
              <div className="flex gap-2 mt-2">
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={hasGlasses} onChange={(e) => setHasGlasses(e.target.checked)} /> Glasses</label>
                <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={hasBeard} onChange={(e) => setHasBeard(e.target.checked)} /> Beard</label>
              </div>
              ...
              {uploadedPhoto && <img src={uploadedPhoto} alt="uploaded preview" className="mt-2 w-full h-24 object-cover rounded-md border" />}
            </div>
          </div>
        </section>
        ...
        <section id="about" className="mt-8">
          <ContentCard title="About" onVisit={() => handleSectionClick('about')}>
            <p>I specialise in operationalising complex processes, building scalable playbooks and driving outcome-oriented execution across cross-functional teams.</p>
          </ContentCard>
        </section>
        ...
        <section id="contact" className="mt-6 mb-12">
          <ContentCard title="Contact" onVisit={() => handleSectionClick('contact')}>
            <p>Interested in working together? Reach out at <a href="mailto:youremail@example.com" className="text-indigo-600">youremail@example.com</a></p>
          </ContentCard>
        </section>
      </main>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100">
      <div className="flex items-start justify-between">
        ...
        <div>
          <button onClick={onVisit} className="text-sm px-3 py-2 bg-slate-50 rounded-md border">Bring avatar</button>
        </div>
      </div>
...
function ExperienceList() {
  const experiences = [
    { role: "Senior Process Manager", company: "eClerx (applied)", period: "2025 - Present", bullets: ["Led process optimization for Ops", "Built playbooks used across squads"] },
    { role: "Process Manager", company: "Opendoor", period: "2021 - 2024", bullets: ["Scaled QA automation", "Reduced turnaround time by 18%"] },
  ];
  return (
        ...
        <div key={i} className="p-4 rounded-lg bg-slate-50 border border-slate-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-slate-500">{e.company} • {e.period}</div>
            </div>
          </div>
...
function AvatarSVG({ skinTone, hairColor, hasBeard, hasGlasses, uploadedPhoto }) {
  // If uploadedPhoto is present, display circular cropped photo with small animated border
  if (uploadedPhoto) {
    return (
    ...
    <div className="w-32 h-32 relative" style={{ transform: 'translateY(-6px)' }}>
      <svg viewBox="0 0 220 220" className="w-full h-full" preserveAspectRatio="xMidYMid meet">
        ...
        <g style={{ transform: 'translate(0,0)' }}>
          ...
          {/* beard */}
          {hasBeard && (
            <path d="M70 140 q40 30 80 0 q-10 20 -80 0" fill="#2b2b2b" opacity="0.9" />
          )}
